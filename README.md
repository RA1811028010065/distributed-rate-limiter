...
This launches a NATS server and the rate-limiter service (which will synchronise buckets using the shared NATS instance) in detached mode, so your shell remains usable during the run. The Makefile drives `docker compose up --detach --build --quiet-pull --remove-orphans` and then invokes `hack/wait-compose.sh` to poll the health checks defined in `deploy/docker-compose.yml`. That script streams container health every few seconds (without depending on the experimental Compose `--wait` flag that has triggered runtime incompatibilities on some hosts) and prints diagnostics plus logs if any service becomes unhealthy. Tail the stack output any time with `make compose-logs` and stop it with `make compose-down`.
