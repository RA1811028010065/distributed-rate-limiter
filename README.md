...
1. **Provision Kind (optional)** – creates a cluster on its own Docker network so SSH routes remain untouched:
   ...
   ```
   The helper configures the `ratelimiter-kind` bridge on `10.240.0.0/16` and reuses it across runs.
...
3. **Load the image into Kind** (skip this when the cluster can pull from your registry directly):
   ...
   ```
4. **Deploy manifests** – remember to update `deploy/kubernetes/rate-limiter.yaml` with the image reference you built:
   ```bash
...
7. **Clean up** – remove manifests and optionally the Kind cluster when you are done:
   ...
   ```
8. **Automated smoke** – mirrors the CI pipeline if you want a scripted confidence check:
   ```bash
...
1. **Install or upgrade** – point the chart at the image you built and the namespace you want to use:
   ```bash
   ...
   make helm-install
   ```
   If you are using Kind or another air-gapped cluster, run `make kind-load IMAGE=rate-limiter:local KIND_CLUSTER=rate-limiter` beforehand so the nodes already contain the image tag referenced in your values.
